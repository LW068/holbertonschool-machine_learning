...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        # Open the class names file to get the corresponding class name for each index
        with open(self.class_names, 'r') as file:
            class_names = file.read().strip().split('\n')

        # Iterate through the boxes to draw them on the image
        for i, box in enumerate(boxes):
            class_name = class_names[box_classes[i]]
            ...
            # Draw the rectangle with a blue line of thickness 2
            cv2.rectangle(image, (x1, y1), (x2, y2), (255, 0, 0), 2)

            # Position the text 5 pixels above the top left corner of the box
            text_position = (x1, y1 - 5)

            # Draw the class name and box score in red
            cv2.putText(image, text, text_position, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)

        # Display the image in a window with the name as file_name
    ...
    def predict(self, folder_path):
        """Predict the bounding boxes, class indices, and scores for images in a folder."""
        ...
        for i, image in enumerate(images):
            ...
            pimages, _ = self.preprocess_images([image])
        
            # Predict boxes, box_classes, and box_scores
            boxes, box_classes, box_scores = self.your_method_for_prediction(pimages) # Assuming you have a method to predict boxes

            # Append the predictions to the list
            predictions.append((boxes, box_classes, box_scores))

            # Get the filename without the full path for window naming
            file_name = os.path.basename(image_paths[i])

            # Show the boxes using the previously defined method
